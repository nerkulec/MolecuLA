...
class VaeTransformer(nn.Module):
    ...
    def decode(self, z, x=None, mode='eval', sos_id=1):
        ...
        pred_len_i = torch.round(pred_len).long().clamp(max=self.max_len)
        
        z = self.fc_z2h(z)
        slots = z.view(B, self.num_slots, self.hidden_size)
        # During training the reconstruction target, not the still-learning
        # length head, defines the decode canvas.  The old behavior dropped
        # every target position beyond the predicted length from the loss.
